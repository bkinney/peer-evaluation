...
if(true){//$ptype==0 - categories get a bit weird if both, but we'll let users sort that out for themselves
	...
	echo '<p title="Data for comparing the number of peer evaluations completed by each student against his/her group size. Please note that the required submissions will be one less than the group size when self-evaluation is not required." ><span style="color:orange">NEW!</span> <a href="'  .$peerhtml . 'accountability.php?proj=' . $_GET['proj']  .  '&facid=' . $_GET['facid'] . '">Download Compliance Data</a></p>';
	}
echo '</div>';	
mysqli_close($link);
	exit();
