...
if($_SESSION['facid'] == $_GET['facid']){//identity confirmed//valid login
		...
		$uri = "/api/v1/courses/". $_SESSION['_basic_lti_context']['custom_canvas_course_id'] ."/assignments";
			$token = $_SESSION['token_arr']['domain'];
	...
	if(get_val($_GET,'endpoint')){
		...
		echo '</p><p>percent completed: ' . $queueStatus['completion'] . '</p>';
			...
			echo '<a href="'  .$peerhtml . 'postGradesToCanvas.php?endpoint='. $endpoint.'&facid=' . $_GET['facid'].'" >Check again</a>';
...
$update = $api->post_canvas($args,"POST");
		//echo '<br>';
		if($update)echo 'Your grades have been queued for posting. If you have a large student roster, it may take a few minutes to complete. You may safely exit this interface or close your browser without interrupting the update.  <a href="'  .$peerhtml . 'postGradesToCanvas.php?facid=' . $_GET['facid'] . '&endpoint='.$update['url'].'">Check status</a>';
