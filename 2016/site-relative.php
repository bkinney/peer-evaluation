...
<!-- 
dependencies:
loaded: (has no direct access to php vars)
getroster
getgroups
defaultInstructions
studentResults7
...
studentData3 (local)
includes/studentData3[includes/db_peer_mysqli.php] [peer_evals, product_evals, student2], 
studentResults7[includes/db_peer_mysqli.php, submitEval2, studentform5] [grades2, student_evals_all, product_evals_all, assigned_evals, studentall, productall, glue_assigned2],
profData5[includes/db_peer_mysqli.php, includes/mysessions] [instructor5, grades3],
email2_1[timeout, email2] [ glue_assigned2],
summaryTable9[includes/db_peer_mysqli.php] [],
emailGroup[timeout,db_peer_mysqli.php],
studentGroups[db_peer_mysqli.php],
exportSakai6[db_peer_mysqli.php] [pending, instructor4, grades],
submitEval[db_peer_mysqli.php] ,
 /canvas/ping,
...
---------------2016----------
/peer dependencies
mu.php - for TA access outside of Canvas
index.php - all the links point here.
config.php - controls where index points
student.php - student link, just sets some variables and includes the index
m.php - includes the current m.php - be sure to keep the latter updated. 
 /js subfolder
 /Styles subfolder
 hopefully other than  these, all dependent files are in the current year subfolder
...
require_once("/www/peer/2016/peerpaths.php");//site-relative path to version folder
require_once("/www/canvas/sitepaths.php");//site relative paths to canvas folder
if(!isset($context)){
...
//stuff cas needs
	...
	$secret="myrealsecret";
	//include "/www/git/canvas/auth-fork-token.php";
	//if(!isset($loggedin)) include $canvasphp . "auth-fork-token.php";
	//$blti = $context->valid;
	
	$lti = isset($_POST['lis_person_sourcedid']) ;
                if ($lti  ){//bypass cas
				
					$tokentype="domain";
					$secret="myrealsecret";
					$testing=false;
					//$include=$peerphp . "site-relative.php";
					//echo $canvasphp . "all_purpose.php" . $peer_testing;
					//include $canvasphp . "canvas_include.php";
					include_once( $canvasphp . "all_purpose.php");
					
					$blti=$context->valid;
					
					//exit();
				}else if(!isset($_SESSION['_basic_lti_context'])){
...
if($blti){
	...
	$domain=$context->info['custom_canvas_api_domain'];
		if(($c_instructor || $c_ta) && $canvas)$canvasgroups = $toolserver . $peerhtml ."getgroups.php?courseid=" . $courseid . "&custom_canvas_api_domain=" .$domain;
	$canvasrubrics = $toolserver . $peerhtml ."getoutcomegroups.php?courseid=" . $courseid . "&custom_canvas_api_domain=" . $domain;
...
if( preg_match("/FACULTY/",$usertype) || preg_match("/(?<!MISC_)STAFF/",$usertype)){
	$usertype="FACULTY STUDENT";
	$peer_testing=true;//not the same as debug boolean. do not overwrite!
...
//if($usertype !=  "loggedout")$peer_testing = true;
//if( preg_match("/FACULTY/",$usertype) || preg_match("/STAFF/",$usertype))$peer_testing=true;
if($peer_testing){
	$studentid = $emplid;
	$facid = $emplid . "9";
...
<link id="togglestyle" href="//apps.ats.udel.edu/peer/Styles/2015.css" rel="stylesheet" type="text/css" />
<?php if ($canvas) : ?>
<script type="text/javascript" src="<?php echo $peerhtml ?>canvas_functions.js"></script>
<?php endif ?>
...
<div class="vscroll login">  <ul class="formlist">
  ...
  <?php include $peerphp . "footer2.php" ?>
<script>
...
Welcome, <?php echo $firstName .' <a href="/peer/index.php?logout=true"'?>>[ Log Out ]</a>
<?php endif ?>
...
<div id="tabs">
    ...
    <div id="logout"></div>
    <div id="fachelp"><iframe width="100%" height="1500" scrolling="auto" src="<?php echo $peerhtml ?>help3.php" style="border:none"></iframe> </div>
...
<p>Each project allows a single evaluation event, in which all students within a group evaluate each other and themselves. Future versions will allow for the creation of multiple evaluation events per project, to encourage improvement over the course of a single project.</p>
 ...
 <form id="submitEval" >
  <p>
    ...
    <input name="instructorID" id="instructorID" type="hidden" value="<?php echo ($peer_testing) ? $facid : $emplid  ?>">
  </p>
...
<div id="custominstructions" contenteditable ><?php include $peerphp . "defaultInstructions.php" ?></div><!-- end custominstructions-->
...
<div id="pcustominstructions" contenteditable ><?php include $peerphp . "pdefaultInstructions.php" ?></div><!-- end custominstructions-->
    <blockquote><p class="instructions">The remaining fields will appear beneath the instructions in every evaluation, and can not be customized. Total score will be summed automatically as component scores are entered.</p>
    <p>
      Total score: 
        <input name="grade" type="text" value=""  disabled >
...
<div id="stabs">
	<ul>
        ...
        <li class="student"><a href="#summaryview">My Grades</a></li>
        <li><a href="/peer/student-help.php" >Help</a></li>		
	<li><a href="#logout" >Log Out</a></li>
	...
	<div id="summaryview" class="fullheight"> 
<h1>Feedback for <?php echo $firstName . " " . $lastName ?></h1>
		<div id="summaryResults"><?php include $peerphp . "/showGrades2.php" ?></div>
    </div>
...
if($peer_testing){
	...
	echo '<div id="stuData">';
	include $peerphp . "/studentData3.php";//new local file. compare with studentData3.php
	echo '</div></div>';
...
<p>  <textarea name="body" id="body" cols="70" rows="5"></textarea>
 ...
 <div id="results" class="rounded" ><h3 class="ui-widget-header ui-corner-top">What&apos;s New</h3>
   ...
   <h4>Summer 2016 Updates</h4>
 <ul>
 
    <li>Rosters from a standalone project can be imported into a project owned by a Canvas course.</li>
...
function clearInstructorForm(defaultGroup){
		...
		$("#projectName").val("");
		$("#projectName-w").val("");
		$("#custominstructions").load("<?php echo $peerhtml ?>defaultInstructions.php");
		$("#pcustominstructions").load("<?php echo $peerhtml ?>pdefaultInstructions.php");
...
function email2(){
	...
	$("#ajax").load("<?php echo $peerhtml ?>email3.php",obj,function(response,status,xhr){
		    if (response == "error") {
...
function showInstructorForm(n,deleteOrig,confirmed){
	...
	if(n){//this is an edit or copy. no wizard
		...
		$("#custominstructions").load("<?php echo $peerhtml ?>studentResults7.php",postdata,function(){//show person as evaluatee to get peer instructions
...
/*				$("#leftside").addClass("hidden");
			...
			postdata.id=1;
			postdata.emplid = '<?php echo $_SESSION['cas_data']['EMPLID'] ?>';
			if(($(ref).attr("data-type"))!=0)$("#pcustominstructions").load("<?php echo $peerhtml ?>studentResults7.php",postdata);//show product as evaluatee to get product instructions
			enableDrag();
		});
...
function getNames(roster,groupname){
	...
	$("#ajax").load("<?php echo $peerhtml ?>getNames3.php",data,function(response, status, xhr) {
...
function deleteProject(n){
	...
	$("#results").load("<?php echo $peerhtml ?>profData5.php",obj,function(){
		activegroup=null;														   
...
$("#summary").html('<h3 class="ui-widget-header ui-corner-top">Project Summary</h3><iframe  src="<?php echo $peerhtml ?>summaryTable9.php?proj='+n+'&facid=<?php echo $facid ?>" height="640" width="96%" frameborder="0" marginheight="0" marginwidth="0"></iframe>');
...
function testAs(s){
	...
	if(ta) currentRole="faculty";
		...
		$("#summaryResults").load("<?php echo $peerhtml ?>showGrades2.php",obj);
		$("#stuData").load("<?php echo $peerhtml ?>studentData3.php",obj,function(){
		...
		obj.emplid='<?php echo $_SESSION['cas_data']['EMPLID']; ?>';
		$("#facdata").load("<?php echo $peerhtml ?>profData5.php",obj,function(){
			initList();
		});
	}
...
var testing = '<?php echo $peer_testing?>';
/**
...
$(document).ready(function(e) {
	...
	$(".step").find("p:eq(0)").append(' <button> <a href="<?php echo $peerhtml ?>help3.php" title="open help in new window" target="help"> ? </a></button>');
...
/*						$("#results").hide();
	...
	//changelog - hitting a different ping. Would be a duplication, except we are not printing the universal header here
   	var lastUpdate = 0;
    var checkInterval = setInterval(function(){
       if(new Date().getTime() - lastUpdate > 840000){// 14 mins * 60 * 1000
           clearInterval(checkInterval);
		   
       }else{   
            $.get('/canvas/ping.php');
       }
    }, 299000); // just under 5 minutes
...
function launchWizard(){
	...
	$("#step1a").dialog({
		...
		buttons:{
			...
			close: function(){
				$("#editor").detach().appendTo("#eplaceholder");
			}
	...
	$("#step3a").dialog({
		...
		buttons:{
			...
			close: function(){
				$("#peditor").detach().appendTo("#pplaceholder");
			}
...
params  = 'width='+screen.width*.9;
 ...
 params += ', scrollbars=1';
 params += ', resizable=1';
	window.open("<?php echo $peerhtml ?>studentGroups.php?proj="+n,"popup",params,false);
...
function releaseGrades(tiny){
	...
	$("#results").load("<?php echo $peerhtml ?>studentResults7.php",pd,function(){
		var obj = new Object();
		obj.emplid='<?php echo $_SESSION['cas_data']['EMPLID']; ?>';//refresh the projects menu
					$("#facdata").load("<?php echo $peerhtml ?>profData5.php",obj,function(){//profData4s
			//initList();
			testAs("faculty");
			if(tiny){
...
$("ul.formlist ul").delegate("li:not('.label,.archived')","click",function(e){
		...
		pd.emplid='<?php echo $_SESSION['cas_data']['EMPLID'] ?>';
		$("#results").load("<?php echo $peerhtml ?>studentResults7.php",pd,function(){$("#results").show();
...
*/
function emailGroup(n){
	$("#email").html('<a  href="#" class="btn rounded exit" onclick=\'$("#email").hide()\'>X</a><iframe src="<?php echo $peerhtml ?>emailgroup.php?projectID='+n+'" height="300" width="600" frameborder="0" ></iframe>').show();
}
/**
...
function exportOptions(n,f){
	...
	$("#exportoptions").text('loading, please wait').dialog("open").load('<?php echo $peerhtml ?>exportSakai6.php?facid=' + f + '&proj=' +n+ '&type=unknown').dialog("open");
}
/**
...
function exportAnalytics(n,f){
	...
	$("#exportoptions").text('loading, please wait').dialog("open").load('<?php echo $peerhtml ?>analytics2.php?facid=' + f + '&proj=' +n+ '&type=unknown');;
	//$("#exportoptions")
/*	//$("#exportoptions").load('<?php echo $peerhtml ?>analytics.php?facid=' + f + '&proj=' +n+ '&type=unknown',function(){
...
function populateEmail(){
	...
	if($("#delay").hasClass("errmsg")){
		...
		obj.emplid = '<?php echo $_SESSION['cas_data']['EMPLID'] ?>';
		console.log(obj);
		$("#delay").load("<?php echo $peerhtml ?>email2_1.php",obj,function(){
