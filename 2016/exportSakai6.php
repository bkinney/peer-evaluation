...
if($ptype!=1 && strpos($facid,'can')!== false){
	//changelog - bug - missing a quote in iframe src
	echo '<p><span style="color:orange">NEW! </span><a title="automatically create and populate a Canvas gradebook column with summary grades and comments" href='  .$_SESSION['peerhtml'] . 'postGradesToCanvas.php?proj=' . $_GET['proj']. '&facid=' . $_GET['facid']. '&maxscore=' . $maxscore . '" target="canpost">Post Peer Grades to Canvas Gradebook</a></p><iframe src="'  .$_SESSION['peerhtml'] . 'pending.html" id="canpost" name="canpost" width="100%" height="80" border="0" scrolling="auto"></iframe>';
}
echo '<p><a href='  .$_SESSION['peerhtml'] . 'exportCanvas.php?type=0&proj=' . $_GET['proj']. '&facid=' . $_GET['facid']. '&maxscore=' . $maxscore . '" title="Export Canvas compliant csv. You will not need this link if you are launching Peer Evaluation from within Canvas. This legacy download includes grades, but not comments.">Download Peer Grades for Canvas</a></p>';
if($ptype !=1){
	echo '<p><a href="'  .$_SESSION['peerhtml'] . 'exportSakai6.php?type=0&proj=' . $_GET['proj']  .  '&maxscore2=' . $maxscore_p . '&facid=' . $_GET['facid'] . '">Download Peer Grades with Comments</a></p>';
	
}
echo '<div style="color:#014598">';
if ($ptype!=0){
	echo '<p><a href='  .$_SESSION['peerhtml'] . 'exportSakai6.php?type=1&proj=' . $_GET['proj'] . '&maxscore=' . $maxscore .  '&maxscore2=' . $maxscore_p .  '&facid=' . $_GET['facid'] . '">Download Product Grades</a> (instructor submitted)</p>';
	echo '<p><a href='  .$_SESSION['peerhtml'] . 'exportSakai6.php?type=1.5&proj=' . $_GET['proj'] . '&maxscore=' . $maxscore .  '&maxscore2=' . $maxscore_p .  '&facid=' . $_GET['facid'] . '">Download Product Evaluations</a> (all student submissions)</p>';
}
if ($ptype==2){
	echo '<p><a href='  .$_SESSION['peerhtml'] . 'exportSakai6.php?type=2&proj=' . $_GET['proj'] . '&maxscore=' . $maxscore .  '&maxscore2=' . $maxscore_p .  '&facid=' . $_GET['facid'] . '">Download Combined Grades</a></p>';
}
